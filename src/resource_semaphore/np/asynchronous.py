...
class _NpAsyncWaiter:
    seq: int
    demands_arr: np.ndarray
    event: asyncio.Event


class AsyncNumpyResourceSemaphore(BaseAsyncResourceSemaphore[K]):
    """
    Numpy-backed multi-resource async semaphore.
    """
    ...
    def __init__(self, resources: Mapping[K, int]):
        ...
        self._waiters: list[_NpAsyncWaiter] = []
    ...
    async def acquire(
        ...
        waiter = _NpAsyncWaiter(seq=seq, demands_arr=demands_arr, event=asyncio.Event())
